...
public struct MarketTypePolicyCreatedEvent has copy, drop {
    policy_id: ID,
    trust_tier: u8,
    market_type: u8,
    version: u64,
...
public struct ResolverPolicyCreatedEvent has copy, drop {
    policy_id: ID,
    version: u64,
}

/// Defines allowed market structures for a specific trust tier + market type combination.
public struct PMMarketTypePolicy has key, store {
    id: UID,
    version: u64,
    name: String,
    trust_tier: u8,
...
public struct PMResolverPolicy has key, store {
    id: UID,
    version: u64,
...
public fun create_market_type_policy(
    ...
    event::emit(MarketTypePolicyCreatedEvent {
...
public fun validate_market_against_policy(
...
public fun create_resolver_policy(
    _admin: &prediction_market::pm_registry::PMAdminCap,
    dispute_window_deterministic_ms: u64,
    dispute_window_declared_ms: u64,
    dispute_window_creator_ms: u64,
    ...
    event::emit(ResolverPolicyCreatedEvent {
        policy_id: object::id(&policy),
        version: 1,
    });
...
public fun policy_version(p: &PMMarketTypePolicy): u64 { p.version }
public fun policy_trust_tier(p: &PMMarketTypePolicy): u8 { p.trust_tier }
public fun policy_market_type(p: &PMMarketTypePolicy): u8 { p.market_type }
public fun policy_resolution_class(p: &PMMarketTypePolicy): u8 { p.resolution_class }
public fun policy_required_outcome_count(p: &PMMarketTypePolicy): u16 { p.required_outcome_count }
public fun policy_max_outcomes(p: &PMMarketTypePolicy): u16 { p.max_outcomes }
public fun policy_required_source_class(p: &PMMarketTypePolicy): u8 { p.required_source_class }
public fun policy_required_evidence_format(p: &PMMarketTypePolicy): u8 { p.required_evidence_format }
public fun policy_is_active(p: &PMMarketTypePolicy): bool { p.active }

public fun resolver_version(p: &PMResolverPolicy): u64 { p.version }
public fun resolver_dispute_window_for_class(p: &PMResolverPolicy, resolution_class: u8): u64 {
...
public fun resolver_escalation_timeout_ms(p: &PMResolverPolicy): u64 { p.escalation_timeout_ms }
public fun resolver_is_active(p: &PMResolverPolicy): bool { p.active }
...
public fun deactivate_market_type_policy(
    policy: &mut PMMarketTypePolicy,
    _admin: &prediction_market::pm_registry::PMAdminCap,
) {
    policy.active = false;
}

public fun deactivate_resolver_policy(
    policy: &mut PMResolverPolicy,
    _admin: &prediction_market::pm_registry::PMAdminCap,
) {
    policy.active = false;
}
