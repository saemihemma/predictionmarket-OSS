...
import { COLLATERAL_SCALE } from "./collateral";

function toDisplayUnits(amount: bigint): number {
  return Number(amount) / Number(COLLATERAL_SCALE);
}

export function formatNumber(n: bigint): string {
  const num = toDisplayUnits(n);
...
export function formatPnL(n: bigint): string {
  ...
  return prefix + num.toFixed(Math.abs(num) >= 100 ? 0 : 2).replace(/\.00$/, "");
}
